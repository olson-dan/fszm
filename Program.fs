...
type Machine(filename) = class
    ...
    (* test_attr *) (fun i x y ret ->
        let o = x |> s.vin_addr |> s.readObj
        (o.attrib &&& (attr_bit y)) <> 0u |> jump i);
    (* set_attr *) (fun _ x y _ ->
        let o = x |> s.vin_addr |> s.readObj
